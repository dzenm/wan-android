...
abstract class BaseAdapter<T : Any> : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun getItemCount(): Int = mBeans?.size + Companion.ITEM_FOOTER_COUNT
        ?: 0
    ...
    override fun getItemViewType(position: Int): Int {
        if (position + 1 == itemCount) {
            return Companion.ITEM_TYPE_FOOTER
        }
        return Companion.ITEM_TYPE_CONTENT
    }
    ...
    fun setFooterState(holder: FooterViewHolder) {
        when (loadStatus) {
            Companion.LOAD_STATUS_LOADING -> {
                holder.progress.visibility = View.VISIBLE
                holder.tip.setText("正在加载中")
            }
            Companion.LOAD_STATUS_COMPLETE -> {
                holder.progress.visibility = View.GONE
                holder.tip.setText("加载更多")
            }
            Companion.LOAD_STATUS_END -> {
                holder.progress.visibility = View.GONE
