        ...
        observable.subscribeOn(Schedulers.io())
            ...
            .subscribe(object : Observer<BaseModel<T>> {
                ...
                override fun onComplete() {
                    Logger.d(activity::class.java.simpleName + " --- 请求完成: ")
                    dismiss()
                ...
                override fun onSubscribe(d: Disposable) {
                    Logger.d(activity::class.java.simpleName + " --- 取消订阅: ")
                    dismiss()
                ...
                override fun onNext(t: BaseModel<T>) {
                    dismiss()
                    if (t?.errorCode == 0) {
                        Logger.d(activity::class.java.simpleName + " --- 请求成功: " + t.toString())
                        onCallback.let { it.onNext(t.data) }
                    } else {
                        Logger.e(activity::class.java.simpleName + " --- 服务端返回错误: " + t?.errorCode)
                ...
                override fun onError(e: Throwable) {
                    Logger.e(activity::class.java.simpleName + " --- 请求错误: " + e)
                    dismiss()
                    onCallback.let {
                        it.onError(e)
                        onComplete()
